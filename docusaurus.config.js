...
const redirects = [
  ...
  },
  {
      ...
      to: '/eigenlayer/concepts/whitepaper',
  },
  {
...
// Slashing

  {
    ...
    to: '/eigenlayer/developers/howto/build/slashing/implement-slashing'
  },
...

  {
      ...
      to: '/eigenlayer/reference/economy/sidecar',
  },
  
  // Legal
  {
      from: '/eigenlayer/legal/disclaimers',
      to: '/eigencloud/legal/disclaimers',
  },
  {
  ...
  // Reference
  {
      ...
      to: '/eigenlayer/concepts/operator-sets/allocation-deallocation',
  },
  ...
  },
  {
      ...
      to: '/eigenlayer/reference/learning-resources',
  },
  ...
  {
     from: '/eigenlayer/developers/howto/get-started/implement-minimum-onchain-components',
     to: '/eigenlayer/developers/howto/get-started-without-devkit/implement-minimum-onchain-components',
  },
  {
     from: '/eigenlayer/developers/howto/get-started/quickstart',
        ...
        [
          "@docusaurus/plugin-client-redirects",
          {
            ...
            createRedirects(existingPath) {
              ...
              //
              // 1. Standard folder mirroring:
              //    If there is a real doc at `/eigenlayer/foo`, generate:
              //    `/products/eigenlayer/foo`
              //
              for (const [oldDir, newDir] of folderMoves) {
                if (existingPath.startsWith(newDir)) {
                  results.push(existingPath.replace(newDir, oldDir));
                }
              }

              //
              // 2. Manual redirects whose `to:` exactly matches a real route
              //
              for (const r of redirects) {
                if (r.to === existingPath) {
                  results.push(r.from);
                }
              }

              //
              // 3. Manual redirects whose `to:` is under /products/*,
              //    and should map to the *real* route under /<product>/*.
              //
              for (const r of redirects) {
                if (!r.to.startsWith('/products/')) continue; // <-- essential

                for (const [oldDir, newDir] of folderMoves) {
                  if (r.to.startsWith(oldDir)) {
                    const realTarget = r.to.replace(oldDir, newDir);

                    if (realTarget === existingPath) {
                      results.push(r.from);
                    }
                  }
                }
              ...
              //
              // 4. Return unique results only (prevents all duplicate warnings)
              //
