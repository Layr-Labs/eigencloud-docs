...
While in Alpha **an allowlisted account is required to create apps.** Use an existing address with `eigenx auth login` or generate a new address with `eigenx auth generate`, then submit an onboarding request [here](https://onboarding.eigencloud.xyz/).
:::
